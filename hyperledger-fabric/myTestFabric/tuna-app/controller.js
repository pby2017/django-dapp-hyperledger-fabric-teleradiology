	...
	return {
		...
		get_opinion: function (req, res) {
			...
			Fabric_Client.newDefaultKeyValueStore({
				...
				// queryOpinion - requires 1 argument, ex: args: ['4'],
				const request = {
					chaincodeId: 'tuna-app',
					txId: tx_id,
					fcn: 'queryOpinion',
				...
				if (query_responses && query_responses.length == 1) {
					if (query_responses[0] instanceof Error) {
						console.error("error from query = ", query_responses[0]);
						res.send("Could not locate opinion")
		...
		add_opinion: function (req, res) {
			...
			req.on('end', () => {
				...
				Fabric_Client.newDefaultKeyValueStore({
					...
					// recordOpinion - requires 2 args, ID, opinion - ex: args: ['1', 'someone's opinion'], 
					// send proposal to endorser
					const request = {
						//targets : --- letting this default to the peers assigned to the channel
						chaincodeId: 'tuna-app',
					...
					if (isProposalGood) {
						...
						// let data = fs.readFileSync(path.join(__dirname, '../basic-network/crypto-config/peerOrganizations/org1.example.com/msp/tlscacerts/tlsca.org1.example.com-cert.pem'));
