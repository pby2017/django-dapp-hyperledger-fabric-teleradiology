...
func (s *SmartContract) Invoke(APIstub shim.ChaincodeStubInterface) sc.Response {
	...
	} else if function == "queryOpinion" {
		return s.queryOpinion(APIstub, args)
	} else if function == "recordOpinion" {
		return s.recordOpinion(APIstub, args)
	}
...
func (s *SmartContract) queryTuna(APIstub shim.ChaincodeStubInterface, args []string) sc.Response {
	...
	// tunaAsBytes, _ := APIstub.GetHistoryForKey(args[0])
	if tunaAsBytes == nil {
...
It takes one argument -- the key for the tuna in question
 */
 func (s *SmartContract) queryOpinion(APIstub shim.ChaincodeStubInterface, args []string) sc.Response {

	if len(args) != 1 {
		return shim.Error("Incorrect number of arguments. Expecting 1")
	}

	opinionAsBytes, _ := APIstub.GetState(args[0])
	if opinionAsBytes == nil {
		return shim.Error("Could not locate opinion")
	}
	return shim.Success(opinionAsBytes)
}
...
This method takes in five arguments (attributes to be saved in the ledger). 
 */
 func (s *SmartContract) recordOpinion(APIstub shim.ChaincodeStubInterface, args []string) sc.Response {
	...
	opinionAsBytes, _ := APIstub.GetState(args[0])
	if opinionAsBytes != nil {
		return shim.Error("Could not record opinion")
	}

	var opinion = Opinion{ Opinion: args[1] }

	opinionAsBytes, _ := json.Marshal(opinion)
	err := APIstub.PutState(args[0], opinionAsBytes)
	if err != nil {
		return shim.Error(fmt.Sprintf("Failed to record opinion: %s", args[0]))
	}
